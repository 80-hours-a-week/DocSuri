...
| 🟡 Machinery on develop; env-flip deferred | Personalization shadow→real — US-P4 boost now applies to the live order, gated behind `SEARCH_RERANK_LIVE` (default **shadow**: `rerank_shadow*` metrics always emit, order unchanged). Go-live = flip the env after reviewing real shadow data, no redeploy (**PR #441**); `rerank_shadow.apply_failed` counter added so a boost-calc bug can't silently no-op (**PR #442/#345**). The flip itself is a deliberate v1 defer. **Fact-blocked until v1.14**: prod (v1.13) emits no `rerank_shadow*` metrics yet (the machinery is develop-only; CloudWatch `DocSuri/Production` has only funnel/settings/retention), so the metric review can't begin until the next promote+deploy. Then US-P5 + keywordWeights. | #345 · #441 · #442 |
