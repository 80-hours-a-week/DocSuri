...
> **Updated**: 2026-07-08 — **Phase 2 close-out**: only ops/decisions/blocked remain. **#345 shadow→real is now buildable-complete on develop** — the env-gated live flip (PR #441) + `apply_failed` observability (PR #442) machinery is in; the flip itself stays a deliberate v1 defer pending shadow-metric review. **#344** drain runbook now points at the `migrate.py audit` step (source_tier distribution) it earlier flagged as a required pre-drain target confirmation. Nothing buildable is left dangling on the Phase 2 board.
...
| 🟡 Machinery on develop; env-flip deferred | Personalization shadow→real — US-P4 boost now applies to the live order, gated behind `SEARCH_RERANK_LIVE` (default **shadow**: `rerank_shadow*` metrics always emit, order unchanged). Go-live = flip the env after reviewing real shadow data, no redeploy (**PR #441**); `rerank_shadow.apply_failed` counter added so a boost-calc bug can't silently no-op (**PR #442/#345**). The flip itself is a deliberate v1 defer. Then US-P5 + keywordWeights. | #345 · #441 · #442 |
