...
def _check_sentence_length(text: str) -> dict:
    ...
    violations = [
        {"sentence": sentences[i][:40], "eojeol": lengths[i]}
        for i, l in enumerate(lengths) if l > _EOJEOL_LIMIT
    ]
...
def _build_undergrad_prompt(user_idea: str, papers: list[PaperHit]) -> str:
    ...
    idea_c = idea[:297] + "…" if len(idea := user_idea.strip()) > 300 else idea
    papers_text = "\n".join(
