...
- Constraints, by kind:
  - `TC-{n}` — technical constraint (stack, platform, integration limits)
  - `BC-{n}` — business constraint (timeline, budget, scope)
  - `LC-{n}` — legal/compliance constraint (privacy, licensing, ORCID/identity, data residency)
