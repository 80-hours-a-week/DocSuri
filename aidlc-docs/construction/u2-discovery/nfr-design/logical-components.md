...
graph TD
    ...
    ORC --> ASM[ResultAssembler<br/>카드 7필드·SEC-9 필터]
...
3. **EmbeddingAdapter**(LlmGatewayAdapter 구현) — Bedrock Cohere Multilingual v3, `input_type=search_query`. **mock/real DI.**
...
8. **ResultAssembler** — 폰 카드 7필드·SEC-9 비노출·종단 상태(PBT-09).
