...
```
입력: SearchRequest{query}, RequestContext{authSession, degradationSignal, requestId}
...
4. retrieve             (HybridRetriever)  — Q2=A
   ...
   └─ 후보 0 → 이후 단계 거쳐 AbstainResult로 수렴 (Q4=A, 빈 성공 페이지 아님)
5. rank                 (RelevanceRanker)  — Q3=A, Q10=A
   └─ 병합 점수 내림차순 · 상위 N=20 절단 · LLM 리랭킹 없음(baseline) · 안정 정렬(PBT-03)
...
- 병합 점수 내림차순 정렬 → **상위 N=20 절단**(N 미만이면 가용분만, US-D3). **LLM 리랭킹 없음(baseline)**.
...
- GroundedResults → 카드 DTO 매핑(§domain-entities §5.1 7필드) + ResultMeta(건수·degraded·mode). degradeMode 활성 시 DegradedResultDTO.
- AbstainResult → AbstainDTO. 폰 우선 직렬화(NFR-U1). **내부 점수·디버그 비노출(SEC-9)**. **DTO 라운드트립(PBT-09)**.
- **빈 cards 성공 페이지 금지**(Q4=A): 후보 0/무매치는 §1 파이프라인에서 AbstainResult로 수렴.

---

## 4. 종단 상태 결정 (Q4=A — FR-11)

```
검증 실패            → ValidationErrorDTO   (fail-closed, SEC-15)
verdict=abstain|block → AbstainDTO          (기권 — 날조 0건, 우선순위 최상)
후보 0 / 무매치       → AbstainDTO          ("관련 논문 없음", 빈 성공 아님)
