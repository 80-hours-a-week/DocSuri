...
| **BR-13 (신원 전제)** *(2026-06-30 개정 — 코드 리뷰)* | `POST /api/search`는 **익명 허용**(미인증도 검색 가능). 단 신원은 **U6 게이트웨이가 주입한 principal(`RequestContext.authSession`)에서만** 파생 — **클라이언트 헤더(`X-User-Id`)는 프로덕션에서 신뢰하지 않음**(위조 방지, SEC-8). principal 있음⇒userId로 SearchExecuted 이력 발행; **principal 없음(익명)⇒검색은 실행하되 이력 발행 생략**(SearchExecuted.userId는 FROZEN 비-null). `X-User-Id` 폴백은 standalone mock-first 개발(build_app `dev_user_fallback`) 전용. *(종전 "인증 필수·미인증 게이트웨이 401"을 대체 — 미들웨어 `/api/search` auth-optional 정합.)* | SEC-8, FR-10, Q5=A |
