...
- **`AccountDeleted` 이벤트는 소프트 삭제 시점이 아니라 *유예 경과 후 영구 파기 시점*에 발행한다 (H2 — 유예 동안 owner-scoped 데이터를 보존해 복구 가능하게, 2026-06-24 리뷰).** owner-scoped 데이터 파기는 U3가 타 유닛을 **직접 호출하지 않고** 이 이벤트를 발행하여 U4(라이브러리·저장검색)·U2(이력)가 각자 구독·파기한다(**의존성 역전 — 코드 DAG 비순환**; 분리될 연구 에이전트 유닛은 생성 시 동일 패턴으로 구독 편입). 파기 잡·구독자는 **멱등·재시도·DLQ**를 보장한다. 캐스케이드 **완료 검증·최대 허용 지연·영구 다운 구독자 처리**는 `shared/events.md` `AccountDeleted` 계약을 따른다 **(#1 리뷰 — GDPR 파기 완전성: 구독자 완료 보고 + 미완료 시 경보·수동 재조정)**.
