...
2. `_NUMERIC_MISMATCH_THRESHOLD`를 스윕하며 곡선 작성 → **날조 0 우선, 과민 기권 최소** strict 값 선택.
3. matcher 정밀화(반올림 톨러런스·단위 정규화) 적용 후 false_abstain 감소 재측정.
4. 변경은 `domain/grounding.py`(임계·`_normalize_number`)만 — FROZEN 계약 무변경. 회귀=`test_grounding_eval` 확장.
5. 결과를 NFR Requirements §9·본 스펙에 back-sync.

## 7. 포인터

- 하니스: `backend/modules/summarization/src/summarization/eval/grounding_eval.py`
- 시드(스캐폴드): `…/eval/seed_cases.py` · 회귀: `tests/test_grounding_eval.py`
