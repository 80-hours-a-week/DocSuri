...
- **핵심 용어 보존은 프롬프트 단계에서 강제**(keep-as-is·핵심 매핑 → 프롬프트 주입, 생성 시 일관·문맥 처리) = **강한(prompt-enforced) 용어**. 강한 용어는 베이스 생성 결과를 바꾸므로 캐시 신원(BR-S1 시그니처)에 반영된다.
- **후치환은 사용자 선호 용어에 한정하여 단순 명사를 교정** = **약한 용어**(조사 안전한 단순 명사만, LLM 재호출 0). 약한 후치환은 **캐시에 굽지 않고 읽기시 오버레이**로 적용한다: 논문당 **공유 베이스 번역**(강한-용어 시그니처 키)을 캐시하고, 응답 조립 시 사용자의 약한 용어를 결정적·멱등(PBT-S3)으로 치환해 내보낸다(verbatim 필드 미치환·`fullText` 재투영). 약한 용어가 없으면 무연산.
- **용어 수정의 캐시 효과**: 약한 용어 편집 → **키 불변 → HIT → 재번역 없이 읽기시 반영**(즉시·무료·베이스 공유). 강한 용어 편집 → 시그니처 변경 → 베이스 분기(owner-scoped, 재생성). `glossaryVer` 카운터는 upsert마다 증가·응답에 반환(용어집 편집기 표시용)하나 **캐시 키 입력이 아니다**.
- **시드 용어집 선정 규칙 (한 번 확정·동결)**: 공유 시드는 사전이 아니라 **모든 프롬프트에 실리는 거버넌스 산출물**이다. 근거 기반으로 한 번 확정 후 동결하고, 개인 취향 롱테일은 **개인 용어집이 흡수**한다(시드를 키우지 않는다). 두 부류 규칙:
  - **keep-as-is(영어 유지)**: 고유 모델·아키텍처명·약어(BERT, LoRA, RLHF…). 번역 시 음차·오역 위험 + 원문 대조 필요. 규칙으로 정당화되므로 자명한 이름은 등급 없이 등재 가능.
  - **강한(prompt-enforced) 매핑**: LLM이 **엉뚱한 한국어로 오번역할 위험이 큰 고빈도 음차 개념어만**(attention→'주의', embedding→'매장'). 기준 = 근거 등급 A/B ∧ 강제 실익 ∧ 고빈도. **A-자명어(신경망·역전파…)는 제외**(LLM이 이미 정확·프롬프트만 비대), **C(혼재; gradient)·D(논란; regularization)는 제외**. `fine-tuning`은 매핑 C(파인튜닝/미세조정 혼재)라 **keep-as-is로 강등**해 논란을 우회한다.
  - 확정 시드 = 강한 `{attention→어텐션, embedding→임베딩, latent space→잠재 공간}` · keep-as-is 목록. **근거·A/B/C/D 등급표·출처·제외 사유·keep-as-is 근거는 `glossary-term-decisions.md`.** 시드 편집은 `seedVer` 경로 세그먼트로 자동 무효화(위 §BR-S1).
- **개인 강한 override는 시드 매핑보다 우선(락 없음)**: 개인 강한 용어는 **공유 시드 매핑과 같은 `term_from`도 덮어쓸 수 있다**(그 사용자에게만). 프롬프트 조립 시 사용자 override가 있는 term은 **시드 매핑을 드롭**하고 개인 것만 실어 모순된 이중 매핑을 없앤다(`_glossary_block`). 개인화는 사용자별 산출물(owner-scoped)이라 공유 표준을 흔들지 않고, 주입은 무해화가 막는다. (keep-as-is 표준어 override와 동일 취급 — 일관.)
- **개인 강한 용어의 프롬프트 무해화(Injection)**: 사용자 `term_from/term_to`는 시스템 프롬프트의 "사용자 선호 매핑" 영역에 실리므로, LLM 전달 전 **개행·구분자(`→` `;` `<paper>`/`<segments>` 등) 무해화** 후 주입한다(SEC-5). 약한 용어는 프롬프트 미유입(후치환만).
- **표준 용어집 노출·개인화 UX (BR-S4/UI)**: 번역 응답은 **이 논문에 등장한 시드 표준 용어**를 `translation.standardGlossary`로 싣는다 — keep-as-is 등장분(LLM `keptTerms` ∩ 시드, `translated` 없음=영어 유지) + 매핑 등장분(번역문에 `term_to` 출현, `translated` 채움). 클라이언트는 용어집을 두 그룹으로 렌더한다:
  - **표준 용어**: keep-as-is 표준어·매핑 **모두 탭하면 강한(prompt-enforced) 개인 override**로 저장(전문 번역 재생성). 매핑 칩은 편집기에 **표준값(어텐션)을 미리 채워** 시작(`standardGlossary[].translated`)하고, 사용자가 바꾸면 개인 override가 시드보다 우선.
  - **원어 유지 용어**: 나머지 `keptTerms`(비표준). 탭하면 **약한 개인어**로 저장(읽기시 반영·재번역 없음). LLM이 `keptTerms`에 함께 담는 **수식 표기**(그리스 변수 theta·eta, `W_q`·`L(w+delta)`·`mathbb{E}` 등)는 결정적 필터(`is_glossary_worthy`)로 제거해 키워드/이름만 노출한다. 필터는 **읽기 경로**에서도 적용해(assembler `filter_kept_terms`) 이전에 캐시된 결과까지 정리한다(시드는 필터 통과).
  - 강/약은 용어의 표준 여부로 자동 결정(사용자에게 강약 토글 미노출). 칩 색: **미수정=파랑 / 내가 지정=주황**. 섹션 순서 = 원어 유지 용어 → 표준 용어. 편집 입력칸은 탭한 칩 바로 아래.
...
- **map 동시 실행**: 부분요약(map) 청크는 독립이라 **바운드 동시 실행**(공유 `map_bounded`·순서 보존·fail-fast); partials는 섹션 순서로 reduce에 투입되어 **직렬과 동일 결과**. reduce는 전 partials 의존이라 순차 유지. 지연만 감소, 비용 중립. 요약 map 청크는 크므로(≈30K tok) 워커 상한은 번역보다 보수적(Bedrock TPM). `max_workers=1`=직렬.
...
- **체크 강도 — HARD vs SOFT(개정)**: 빈/잘림·스키마 미완은 **HARD**(전체 기권). 앵커 실재성은 **SOFT** — 검증 못 한 앵커는 **드롭(요약 본문은 유지)**하고 전체 기권하지 않는다.
- **앵커 실재성 = 구조적 target 해석(개정)**: 앵커는 원문 산문의 verbatim 인용이 아니라 **doc-model의 실제 위치(섹션/표/그림)를 가리키는 포인터**다. 요약은 한국어라 모델이 `label`을 지역화/패러프레이즈하므로(예: "free-riding penalty 수식") 영어 원문에 대한 verbatim substring 검사는 실재하는 앵커도 거의 전부 탈락시킨다(관측: 15개 중 1개). 대신 모델이 `target`에 넣는 **raw 위치 문자열을 doc-model의 실제 섹션 타이틀·표/그림 라벨에 정규화 해석**(소문자·`Section:`/`§` 접두 제거·구두점 무시·대소문자 무시)한다. 해석되면 **KEEP + `label`을 매칭된 canonical doc-model 라벨로 재작성**(칩이 실제 블록으로 점프), 안 되면 드롭(soft `anchor_missing`). 날조 위치는 해석 실패로 자연히 드롭되며, **수치 날조 차단은 아래 HARD 수치 게이트가 독립 담당**(앵커 완화가 반환각 보증을 약화시키지 않음).
- 수치 일치는 HARD지만 **비율 기반** — 원문에 없는 수치 비율이 임계(기본 50%)를 넘을 때만 기권(소수 오차 허용, 다수 날조는 차단 — 반환각 유지). fail-closed가 앵커 단위로 적용됨. **matcher 정밀화**: 동등 수치는 정규화로 매칭 — 퍼센트↔분수(95.3%↔0.953)·천단위 구분(1,200↔1200)·**반올림 톨러런스**(표기 정밀도 기준 half-a-ULP 밴드, 95.3↔95.34)로 false-abstain을 줄이되, 밴드는 좁아(반올림 한정) 날조 통과는 막는다. **수치 임계값(0.5) 자체의 재보정은 별개**(held-out 코퍼스 선행 — 데이터 게이트).
...
- **map 동시 실행(지연 최적화)**: 청크는 서로 독립(map-only)이므로 **바운드 동시 실행**(공유 `map_bounded` 헬퍼: 워커 상한·**입력 순서 보존**·예외 fail-fast). 결과는 청크 순서로 병합해 재주입하므로 **직렬과 바이트 동일**(캐시 산출물 불변) — 속도만 향상, 비용 중립(호출 수 동일). 워커 상한은 Bedrock 스로틀/비용 대비 보수적(런타임 튜닝·NFR); `max_workers=1`은 직렬. 청크 LlmUnavailable/미해결 truncation은 전파 → 기존 1회 재시도 후 기권 경로 유지.
