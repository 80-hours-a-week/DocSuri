...
| **BR-29 (전문 추출 소스·형식)** | **결정 D 활성**(FD plan `사후 결정` Q18=D; #139 근원=추출 소스·형식 미설계). 전문은 **arXiv HTML 우선 → PDF 폴백**으로 취득한다 — HTML은 *가장 깨끗한 평문을 뽑는 소스*, PDF는 폴백. **보관=정규화 평문 1종**(`.txt`); 뷰어는 평문 렌더(앵커 하이라이트 유지, BR-SF). **e-print(LaTeX) 미해제 디코딩 금지** — gzip/tar 페이로드를 텍스트로 디코딩하지 않음(현 `arxiv.py:73` 결함 제거). HTML 부재는 폴백(거부 아님); 추출·평문화 실패는 BR-15 분류(PARSE/FETCH). **범위 밖**: HTML 리치 렌더/보관(수식·표·그림 인라인)은 **에셋 패널(FR-17)과 그림·표 표시가 겹쳐** 별도 결정 필요 → 에이전트 단계로 분리. | **D**, Q5=C, SEC-5, BR-15/19/20 |
