...
- **예비 컴포넌트(잠정 — FD에서 정제)**:
  ...
  - `MultiPaperRetriever` — 질의→U2 검색→교차확인 후보 논문 집합("유사논문 검색" 노드)
  - `EvidenceExtractor` — 논문별 doc-model에서 {주장·방법·결과·한계} 추출(U7 6단계 노드 재사용, 앵커 부착)
...
- [ ] **domain-entities.md**
  ...
  - `PaperCandidate` / `CandidateSet` (다논문 검색 결과)
...
- [ ] **business-logic-model.md**
  ...
  - `retrieveCandidates(query, attachmentContext)` (U2 재사용)
  - `extractPaperEvidence(candidate)` (U7 doc-model 추출 노드 재사용)
...
- **AS-2**: U11은 U6 게이트웨이 단일 진입(authn/authz/rate-limit)을 통과한다. **근거화의 단일 권위는 U6**(정책: 근거 없으면 기권·날조 0건), 비용 단일 권위도 U6다. 기본은 `enforce`/`getBudgetState` **소비**이되, 다논문 형상에 맞춰 **U6/shared-ports 확장이 필요하면 한다**(우회·재구현은 금지 — 권위는 U6 유지). 형상 확정은 Q7.
- **AS-3** *(2026-06-24 결정 — doc-model 전문 기반 전환)*: **후보 선정·근거 추출 모두 동일 doc-model에서** 나온다. doc-model을 **eager 생성**(D6 되돌림)하고 **논문 전체(제목+초록+본문, 표=데이터·수식=latex·캡션, +각주·서지메타 보강·DF-6)** 를 담은 뒤, 그 전문을 **OpenSearch 통합 인덱스로 재구축**(제목+초록만 인덱스 폐기, #136 되돌림; U2 검색·U11 공유). 후보 선정=전문 인덱스 검색(A+ 다중쿼리, Q2), 근거 추출=같은 doc-model(block-id 앵커, Q3). **표·수식·캡션이 찾기·근거에 first-class.** 이 전환은 교차 아키텍처 변경이라 **별도 게이트로 관리**(blast-radius: U1·U2·infra·NFR-C1; §6). 논문별 추출은 U7 *배관 노드* 재사용 + **U11 전용 추출 노드**(Q1/Q3). **구조화 인용(출처논문)은 U8 영역·v1 범위 밖**(참고문헌 섹션 텍스트는 doc-model에 평문으로 포함).
...
**[색인·추출 소스 — 2026-06-24 사용자 결정: doc-model 전문 기반 통합 인덱스] ★아키텍처 변경★**: 앞선 "corpus-v1=초록만·doc-model lazy" 전제를 의식적으로 뒤집는다. 결정: **(1) doc-model을 전 논문 eager 생성**(D6 lazy 되돌림)하고 **논문 전체(제목+초록+본문, 표=데이터·수식=latex·그림 캡션, +각주·서지메타 보강 — 게이트 DF-6)** 를 담는다. **(2) 그 doc-model에서 평탄화한 전문을 OpenSearch 통합 인덱스에 색인**(#136 초록만 되돌림 — **현 "제목+초록만" 인덱스 콘텐츠는 사실상 폐기·재구축**). **U2 검색·U11 에이전트가 단일 인덱스 공유.** 색인 소스 = 근거 소스 = **동일 doc-model 하나**(통일). 이유: 표·수식·캡션을 *찾기·근거*에 first-class로. **U11 단독 결정 아님 → 별도 게이트 `docmodel-fulltext-index-pivot-plan.md`로 관리**(blast-radius: U1 인제스천·U2 검색·OpenSearch 노드·NFR-C1 비용 추정 +$100~220/월; §6). 커밋 `982f64a`(전문 임베딩 복원, 미승인)는 본 결정으로 *정식 게이트로 승격*해 제대로 수행(소스=doc-model·infra resize·비용 갱신).
...
[Answer]: **A+** *(사용자 2026-06-24)* — 전문 통합 인덱스(doc-model 기반) 위에서 bounded 다중쿼리(질의·첨부 분해→U2 여러 번→후보 합집합). C(별도 클러스터링 엔진)는 오버엔지니어링으로 기각.
...
[Answer]: **A** *(사용자 2026-06-24 — U6 통합)* — U11 근거화를 **U7과 동일한 문서충실도 계약**으로 설계하고 그 계약을 **`shared/`(U6 단일 권위)로 승격**해 U11이 채택. **U7 백포트는 단계적**(저위험이면 후속). 검색용 `enforce` 확장이 아니라 *문서충실도* 메커니즘. blast-radius: `shared/ports.md`·U6 FD(+U7 백포트 후속).
