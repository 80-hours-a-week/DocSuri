...
- [ ] **business-rules.md**
  ...
  - 모드 B 경계 규칙(v1 미빌드)
  - QT-8 속성 후보·추적성 매트릭스
- [ ] **frontend-components.md** — *생성 여부 = Q17에서 결정*
...
- **AS-3**: 근거 소스는 **owned arXiv AI/ML 코퍼스(전문 멀티청크)·doc-model**이다. 논문별 근거 추출은 U7 파이프라인 §6 6~7단계 노드를 **기본 재사용**하되, 다논문 fan-out/교차확인이 요구하면 U7 노드를 **확장**할 수 있다(blast-radius=U7 FD; Q1/Q3).
...
- **AS-6**: 전용 네비 메뉴·세션 리스트·모드 선택·대화 화면의 **UI 구현은 프런트엔드 책임**이다. U11 FD는 API/도메인 계약까지만 정의한다(구체 화면=프런트 유닛 — Q15).
...
A) **변경 없음 — U2 검색(RRF·PaperId 디덥) top-K 단발 재사용(권장 기본)**. 질의(+첨부 맥락)를 U2에 넣어 상위 후보를 받고 PaperId 디덥. K 상한은 bounded(수치는 NFR). 빈 결과는 빈 성공 금지 — 명시적 기권. U2 무변경.

A+) **확장 — 교차확인 다양성을 위해 bounded 다중쿼리**. 질의·첨부를 하위질의로 분해해 U2를 **여러 번** 호출·후보 합집합(여전히 U2 위, 자율 반복 루프 X). U2 검색 API가 단발만 가정하면 **blast-radius**: U2 인터페이스에 batch/multi-query 진입 추가(U2 FD·`api` 편집) 또는 U11에서 호출만 반복(U2 무변경). 둘 중 어느 쪽인지는 NFR/Code에서 확정.

B) 사용자가 논문을 직접 골라 넣게 한다.

C) 코퍼스 전수 스캔/임베딩 클러스터링으로 후보를 만든다(별도 검색 경로 신설).
...
1. **Functional Design Part 1 = 본 계획서(질문 게이트)** 작성 완료. Q1~Q17 답변 대기.
2. 답변 확정(애매 응답은 후속 질문) 후 **Part 2**에서 `u11-research-agent/functional-design/`의 `domain-entities.md`·`business-logic-model.md`·`business-rules.md`(+ Q15 결과에 따라 `frontend-components.md`)를 생성한다.
3. Functional Design 승인 후 **NFR Requirements**(성능·저장소·외부 API·LLM 모델·보안·운영·테스트 전략)로 진행한다.
4. 커밋·푸시·PR(#170 갱신)은 **사용자 명시 승인 후**에만 진행한다.
