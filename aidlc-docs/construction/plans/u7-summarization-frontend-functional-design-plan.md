...
- **소비 계약(백엔드 U7, 동결)**:
  - 요청 `POST /api/summarize` ← `{ task: "summary"|"translate", paperId, version, persona?: "expert"|"beginner", targetLang?: "ko", abstract? }`
...
B) **요약 우선(US-S1·S5 + 상태)** — 구조화 요약 + 로딩/캐시/기권/비용저하/소스부재 상태만 먼저. 번역(S2)·persona(S4)·출처앵커(S3)는 후속 패스.
...
## Question 4
persona(수준: 전문가용/입문자용, 논문당 최대 2벌) **전환 UX**를 어떻게 할까요? *(Q3=B면 후속으로 미룸)*
...
## Question 6
US-S2 한국어 번역(초록 대상, 응답 `translation:{koreanText, keptTerms[]}`)을 **어떤 면**에서 보여줄까요? *(Q3=B면 후속)*

A) **같은 표면에서 [한국어로] 전환** — 요약 표면 상단에 [요약 | 한국어로] 전환. 번역 뷰는 koreanText + keptTerms(미번역 보존 용어) 배지 표시. 전환은 별도 task 요청(캐시 적용).
