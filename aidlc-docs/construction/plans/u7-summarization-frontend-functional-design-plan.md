...
- **Q3 범위**: 풀세트(US-S1~S5) — 단 **persona 제거 → 단일**, 번역은 **초록 + 전문** 둘 다.
- **Q4 persona**: **폐기**(persona 제거 → 단일).
...
- **여전히 미정(팀/후속)**: **Q2**(진입 목록) · **Q7**(상태 렌더) · **Q8**(식별자 매핑).
...
- **소비 계약(백엔드 U7, 동결)**:
  - 요청 `POST /api/summarize` ← `{ task: "summary"|"translate", paperId, version, scope?: "abstract"|"full", targetLang?: "ko", abstract? }` *(2026-06-19: `persona` 제거, 번역 `scope` 추가 — 기본 abstract, 요약은 full 고정)*
...
[Answer]: ✅ A 풀세트(US-S1~S5) — 단 persona 제거 → 단일, 번역은 초록+전문. (2026-06-19 확정)

## Question 4 — ❌ 폐기 (2026-06-19)
persona(전문가/입문자) **제거 확정**. 요약·번역 모두 단일 수준 → 전환 UX 불필요. 캐시 키에서 `persona` 삭제, 무료 뷰 프리셋(tldr 3줄 등)만 유지.
