...
- **Q2 진입 목록**: A — 검색 결과 + 라이브러리 둘 다 공통 액션.
- **Q3 범위**: 풀세트(US-S1~S5) — **요약에 persona(전문/입문) 유지**, 번역은 **단일**·초록+전문.
- **Q4 persona UX**: A — 표면 상단 세그먼트 토글(전환 시 재요청, 캐시히트 즉시).
- **Q5 출처보기**: **C — 전문 하이라이트 뷰어 신설**. 앵커 클릭 → 원문 전문에서 위치 하이라이트. ⚠️ **신규 백엔드 의존**: 전문 텍스트 반환 API(현재 S3 private·미반환) + OA 라이선스 게이트 + 정규화 텍스트 가독성 처리.
- **Q6 번역**: 상세 페이지 **명시 버튼 2개** — [초록 번역](`scope=abstract`) · [전문 번역](`scope=full`, 신규). 백엔드 계약에 `scope` 추가.
- **Q7 비-해피 상태**: A — 기존 StateView/screenState 재사용.
- **Q8 식별자**: A — `paperId=arxivId`, `version=1`, 추가 본문 없음.
...
> 2. **출처 보기 갭 → Q5=C로 해소(신규 의존 발생)**: US-S3 "원문 하이라이트"를 위해 **전문 하이라이트 뷰어를 신설**한다. 단 백엔드가 앵커 외 **전문 텍스트 반환 API**(현재 S3 private·미반환)를 신설해야 하고, **OA 라이선스 게이트** + 정규화 텍스트 가독성 처리가 따른다.
...
[Answer]: ✅ A 검색 결과 + 라이브러리 둘 다 공통 액션. (2026-06-19 확정)
...
[Answer]: ✅ A 표면 상단 세그먼트 토글 — 전환 시 해당 persona 재요청, 캐시히트 즉시(재생성 0). (2026-06-19 확정)
...
[Answer]: ✅ C 전문 하이라이트 뷰어 신설 — 앵커 클릭 시 원문 전문에서 위치 하이라이트. ⚠️ 신규 백엔드 의존: 전문 텍스트 반환 API + OA 라이선스 게이트 + 정규화 텍스트 가독성. (2026-06-19 확정)
...
[Answer]: ✅ A 기존 StateView/screenState 재사용 + 요약 전용 메시지 매핑. (2026-06-19 확정)
...
[Answer]: ✅ A `paperId=arxivId`·`version=1`·추가 본문 없음. (2026-06-19 확정)
...
## 6. 백엔드 연계 변경 (이번 결정으로 발생 — Code-gen 단계에서 수행)
