...
- [ ] **7.1** 위 산출물을 사용자에게 리뷰 요청 (변경 요청 / 추가 unit 분리 / 합치기 의견 받기).
- [ ] **7.2** 피드백 반영 후 unit 정의를 동결한다.

---

## 8. 후속 단계 연결

- [ ] **8.1** 동결된 unit 정의를 `handoff.md`의 §2 위험·§4 결정에 매핑 갱신 (어느 unit이 어느 위험·결정을 닫는지 명시).
- [ ] **8.2** 다음 설계 단계(API/Data model/Component 설계)의 입력으로 정렬.
