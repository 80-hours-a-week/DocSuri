...
| from \\ to | U1 | U2 | U3 | U4 | U5 | U6 | U7 | U8 | shared |
|---|---|---|---|---|---|---|---|---|---|
...
| **U5 Frontend** | — | sync(REST, 게이트웨이 경유) | sync(REST) | sync(REST) | — | sync(게이트웨이 진입) | sync(REST, 게이트웨이 경유) | sync(REST, 상세보기 경유) | lib(DTO) |
| **U6 Reliability/Ops** | event(인제스천 신호 소비) | sync(게이트웨이→U2 핸들러 호출) | sync(authz 위임 호출) | sync(게이트웨이→U4 핸들러) | — | — | sync(게이트웨이→U7 핸들러) | sync(게이트웨이→U8 핸들러) | lib(이벤트·ports) |
| **U7 Summarization** | (전문 원본 S3 read = capability, 코드 의존 아님) | — | — | — | — | lib(근거화·비용 후크 via `shared/ports`) + event(관측/비용 발행) | — | sync(요약 출처에서 각주 트리 열기) | lib(DTO·ports) |
| **U8 Citation Graph** | — | — | sync(로그인/인가 경로) | sync(노드 저장) | — | lib(레이트리밋·관측 포트) + event(관측 발행) | — | — | lib(DTO·events) |
| **shared** | — | — | — | — | — | — | — | — | — (leaf) |

> U5 사용자 경로는 **U6 게이트웨이를 단일 진입**으로 U2/U3/U4/**U7/U8** 핸들러에 도달한다(표의 U5→U2/U3/U4/U7/U8 sync는 게이트웨이 프런티드).
...
- **결론**: 코드 의존 그래프 비순환(DAG) — U7/U8 추가 후에도 유지. 런타임 호출/이벤트는 단방향 또는 비동기로 순환 없음.
