...
| from | to | kind | purpose |
|---|---|---|---|
| EvidenceChatController | EvidenceChatService | lib | 채팅 턴 오케스트레이션 위임(FR-30, NFR-P5). |
| EvidenceChatController | EvidenceSessionManagementService | lib | 세션 CRUD 오케스트레이션 위임(FR-32). |
...
| EvidenceChatService | EvidenceSessionRepository | lib | 세션 load/create/appendTurn(FR-32). |
| EvidenceSessionManagementService | U3.AuthorizationGuard | lib | **SEC-8 소유권 결정 위임 — 단일 권위 결정점.** |
| EvidenceSessionManagementService | EvidenceSessionRepository | lib | 세션 delete/reset owner-scoped(FR-32). |
| EvidenceAgentOrchestrator | EvidencePaperSearchTool | lib | 논문 검색 도구 호출(자율 오케스트레이션, FR-31). |
| EvidenceAgentOrchestrator | EvidenceDocModelTool | lib | DocModel 블록 읽기 도구 호출(FR-31, FR-18). |
...
| EvidenceFormationService | Job Queue (shared capability) | event | 긴 분석 비동기 잡 발행(NFR-P5, Q9=A). |
