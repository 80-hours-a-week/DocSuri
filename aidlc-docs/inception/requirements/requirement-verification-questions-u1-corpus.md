...
**답변 상태**: ⏳ **준희님 답변 대기**. 각 **[Answer]** 에 재인셉션 차터 기반 **권장안**을 선기입했다. 동의하면 그대로, 바꾸려면 교체하거나 **X) 기타**로 적어주세요.
...
[Answer]: (준희님 결정 — 권장 A. 현행 dedup 키를 코드에서 확인해 정합)
...
[Answer]: A — "완성형" = 구조 완성형(표=데이터·수식=LaTeX·그림 webp 참조). 비전 추론 제외 유지. ※ 비-HTML 폴백 시 표 크롭 이미지 허용 여부는 Q3 추출 품질과 함께 확정.
...
[Answer]: A — 수집 시점 eager 전량. (구) lazy 결정 대체. U7 `SqsDocModelBuildQueue`는 누락분 보강/재빌드 역할로 축소(Q9 버전 갱신과 정합).
...
[Answer]: A — DocModel 기반 인덱싱으로 전환. full-text 청킹은 폐기 또는 폴백으로 격하(Q7에서 청킹 정책 확정).
...
[Answer]: (준희님 결정 — 권장 A. 표/수식 블록의 임베딩 표현은 검색 품질 영향이 커 페이즈 8과 연계)

---

## Q8. Embedding 모델·인덱스 버전 병행

임베딩 모델·인덱스 운영을 무엇으로 하는가?
(현재 코드: `BedrockCohereEmbeddingPort`, `opensearch_index` + `opensearch_index_v2` 병행, `embedding_v2` 존재.)

- **A) 현행 Bedrock Cohere 유지 + v1/v2 인덱스 병행 운영(블루/그린 reindex 경로 유지)**:
  DocModel 기반 전환 시 v2로 재색인 후 컷오버. 모델 교체는 페이즈 8.
- **B) 단일 인덱스로 단순화**.
- **X) 기타**(모델 변경 포함)

[Answer]: (준희님 결정 — 권장 A. DocModel 전환 = v2 재색인 호기로 활용)
...
- **A) 소스별 watermark(예: OAI-PMH from-date / 커서) 저장, 마지막 성공 지점 이후만 수집** (차터 운영):
  전량 재수집 방지. 소스별 독립 워터마크.
- **B) 전량 재스캔 + dedup 의존**.
- **X) 기타**

[Answer]: (준희님 결정 — 권장 A. 코드 `arxiv_oai_set` 테스트 존재 — OAI-PMH 기반 확인)
...
- **A) Scheduler 주기 수집 + 단계별 Retry + 실패 시 DLQ + `ObservabilityHub.emitFailureSignal` 라우팅** (차터/`shared/ports` 정합):
  실패가 인덱싱을 막지 않게 best-effort(코드 주석 "never blocks indexing"). DLQ 재처리 경로 제공.
- **B) 단순 재시도만(별도 DLQ 없음)**.
- **X) 기타**(주기·재시도 횟수·DLQ 보존 구체화)

[Answer]: (준희님 결정 — 권장 A. 주기·재시도 한도·DLQ 보존기간 수치는 NFR에서 확정)
...
[Answer]: (준희님 결정 — 권장 A. 비용 상한 수치·우선순위 규칙은 NFR에서 확정)
