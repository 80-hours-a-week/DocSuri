...
> **개정 (2026-06-23) — Cohere Embed v4.0 마이그레이션 편입**: 명확화 질문 `requirement-verification-questions-v4-migration.md` Q1~Q4 전수 답변(A) 반영 → **FR-18(듀얼 라이트)·NFR-M2(Blue/Green 마이그레이션)·NFR-S2(v4 모델 컷오버)** 등재. 기존 v3 인덱스와의 비호환성을 무중단으로 해결하기 위해 신규 인덱스 백필, 듀얼 라이트, 그리고 Instant Cutover 전략을 확정한다.
...
| **FR-12** | **AI 요약(요약 액션) [U7]**: 검색 결과 카드에서 선택한 **단일 논문의 전문**을 페르소나 질문 기반 **구조화 요약**(핵심주장·기여·방법·결과·한계·재현성)으로 생성. 요약 *수준* 선택(전문가용/입문자용). 각 항목에 원문 근거 앵커(섹션·표·그림) 부기. | 선택 논문 1편에 구조화 요약 반환; 각 주장에 검증 가능한 원문 앵커("출처 보기"); 근거 없으면 날조 대신 기권(FR-5·QT-5); 수준(전문/입문) 선택이 출력에 반영. 온디맨드(NFR-P2). (표·그림 앵커는 FR-17 자산 도입 시 해당 자산으로 연결.) |
...
| **FR-17** | **그림·도표 표시 (멀티모달 — 표시 전용) [U1/U7/U5]**: 인제스천 정규화 시 논문의 **그림·도표를 이미지 자산으로 추출·저장**(추출 방식은 arXiv 소스 가용성에 따른 **혼합** — 소스 있으면 구조화 추출, 없으면 PDF 페이지 크롭 폴백)하고, 상세보기/전문뷰어에 표시한다. 요약/번역의 LLM 입력은 **텍스트+캡션을 유지**하며, 이미지에 대한 **비전 추론은 본 범위 제외**(차기 사이클). | 상세보기/전문뷰어에 원문에서 추출된 **실재 자산만** 표시(생성·합성 이미지 금지 — FR-5 정신); OA 라이선스 미허용 논문은 자산 미노출 + arXiv 링크아웃(BR-SF-11 게이트 재사용); 자산은 **단기 만료 서명 URL**로 서빙(SEC-9/12 — 내부키·토큰 비노출); 표·그림 앵커("출처 보기")는 해당 자산으로 연결(FR-12); 이미지는 lazy-load·플레이스홀더·치수 예약으로 렌더(모바일 성능, 검색 SLA NFR-P1 무관). 온디맨드 표시. |
| **FR-18** | **인제스천 파이프라인 v4 듀얼 라이트 (마이그레이션)**: 마이그레이션 기간 동안 신규 문서는 기존 v3 인덱스(`docsuri-corpus-v1`)와 신규 v4 인덱스(`docsuri-corpus-v2`) 양쪽에 모두 기록(dual-write)된다. | 마이그레이션 완료 시점까지 두 인덱스 모두 최신 상태 유지. |
...
- **QT-5 — 요약/번역 근거화 인수 [U7]**: U7 요약/번역(FR-12/13) 출력도 엄격 근거화 적용 — 보류 평가셋에서 날조 주장/인용 **0건**; 원문에 근거 없는 항목은 **기권**; 각 항목 원문 앵커(섹션·표·그림) 검증 가능. **QT-1 평가셋에 요약/번역 케이스 추가.** U6 GroundingEnforcementHook 재사용 가능성은 Functional Design에서 검증(검색용 설계가 요약 주장에 그대로 적용되는지). (FR-12/13, FR-5와 일관.)
...
| FR-18 듀얼 라이트 (v4 마이그레이션) | v4 마이그레이션 Q3=A |
