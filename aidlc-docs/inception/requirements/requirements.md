...
| **FR-18** | **행동 이벤트 기록 [U9]**: 개인화에 직접 쓰이는 도메인 행동만 owner-scoped 이벤트로 기록한다. 범위는 검색 실행, 논문 상세 조회, 라이브러리 저장/해제, 요약/번역 요청, 출처 보기 클릭, 개인 용어집 수정으로 한정한다. | 백엔드 API 성공 후 확정 기록한다. `library_removed`는 삭제 대상 row를 owner 검증으로 읽고 실제 삭제 성공 후 기록하며, 분석에 필요한 arXiv ID는 삭제 전에 확보한다. 프론트 전용 행동(예: 출처 보기 클릭)만 별도 behavior 이벤트 API로 전송한다. hover/스크롤/일반 UI 클릭 전체 수집은 하지 않는다. |
...
| FR-18 (행동 이벤트 기록) [U9] | U9 개인화 Q2/Q3/Q4/Q5=A (2026-06-23) |
