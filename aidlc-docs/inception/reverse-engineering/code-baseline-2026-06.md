...
| **7** 검색 품질 🟡**착수(재랭킹)** | `discovery/domain/ranker`·`expander`·`reranker`·`ports.RerankAdapter`·`adapters/bedrock_rerank` | **(back-sync 2026-07-06)** Cross-Encoder Reranker 착지 — 공급↔정렬 분리(`Candidate.ranking_score` 정식 필드·ranker는 이 단일 키만 정렬)·별도 어댑터(포트+Bedrock Rerank real+mock)·`domain/reranker`(rerank_width/text/apply)·오케스트레이터 top-M 게이트(예산 off·실패 fail-soft 베이스라인)·retrieve 150 불변. FR-3 개정·BR-5b. 스코프=재랭킹 1건(LTR·클릭로그·query expansion 등 나머지는 제외) | reranker M은 **실측(eval)에 따라 조정**·nDCG@10/MRR/Precision@10 라이브 리프트 실측(신규 eval 하니스+라벨셋·크로스계정)·real 어댑터 Bedrock wire shape 라이브 검증 잔여. **LTR/피드백/클릭로그/query expansion=차기 사이클** |
