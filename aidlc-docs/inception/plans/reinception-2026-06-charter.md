...
> - **상세 라우트 소스 중립 식별자** — 결과 카드는 소스 중립(sourceName/sourceUrl, 페이즈 2/Q2)이나 상세 라우트(`/paper/{id}`)·`PaperMetaVM`은 여전히 `arxivId` 중심. 비-arXiv 전용 논문(arXiv 미수록) 대비 소스 중립 id/라우트 검토(U2 코드리뷰 #3).
