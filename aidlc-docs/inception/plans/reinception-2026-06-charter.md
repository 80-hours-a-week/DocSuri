...
| D2 | **에이전트 유닛 구성** | **2개 유닛으로 분리**(아래 §4): 문헌탐색·근거형성 유닛 / 연구아이디어 유닛. 후자가 전자를 **포트로 의존(Tool 소비)하는 것을 기본 제안**으로 하되, **의존 여부·범위는 requirements 질문지에서 확정**(2026-06-28 완화). *(2유닛 분리 자체는 2026-06-26 승인)* | 담당 분리·상이한 NFR 프로파일. 단방향 의존은 자연스럽지만(5가 4의 근거 산출을 소비) **강제는 아님 → 확정 대상** |
...
| D5 | **페이즈 4·5 병렬 (의존 확정 시)** | **5→4 의존이 확정되는 경우** 페이즈 4·5는 D4 순차의 예외로 **"계약 게이트 병렬 쌍"**으로 진행. 두 유닛 모두 페이즈 1~3 완료 후 **동시 착수**. 전제: ⓐ 에이전트 Tool 포트 계약·근거 출력 DTO를 `shared/`에 **선행 동결**, ⓑ 5는 4의 **녹화 fixture(golden output)**로 개발, ⓒ 5의 "완료"는 **실제 4와의 통합 게이트 통과 후**에만. **의존을 두지 않기로 하면 4·5는 일반 순차/독립 트랙으로 재조정**(D5 병렬 메커니즘 비적용). | 5가 4 *계약*에 의존하는 경우의 병렬 패턴(U2 Mock 선행 재사용) |
...
| **5** | 연구 아이디어 Agent | **석현님** | (신규) | 1·2·3 (+ 4 계약: **의존 확정 시**·병렬·D5) | **그린필드** |
...
**목표**: 다논문을 교차확인해 근거를 비교·정리하고, 근거가 없으면 기권하는 대화형 연구 보조. (5→4 의존 확정 시 **페이즈 5의 하부 Tool**이 된다.)
...
- **페이즈 4(문헌탐색·근거형성 Agent) 의존은 기본 제안**: 새 아이디어의 novelty/Research Gap 판단에 4의 근거 산출이 자연스러운 입력이므로 4를 **Tool로 재사용**하는 것을 기본값으로 둔다. **단 강제는 아니며 의존 여부·범위는 requirements 질문지에서 확정**(독립 시 5가 Search/DocModel/Summary를 직접 소비).
...
> **의존 완화 주의(2026-06-28)**: **2유닛 분리는 확정**이나, 아래 "5→4 단방향 의존"은 **기본 제안**이며
> requirements 질문지에서 최종 확정한다. 본 §4·§4.1은 **의존이 확정될 경우의 설계**를 기술한다.
> 의존을 두지 않기로 하면 §4.1 D5 병렬은 비적용이고 4·5는 일반 순차/독립 트랙으로 재조정한다.

페이즈 4·5를 **두 유닛으로 분리**하고, (의존 확정 시) 5가 4를 포트로 의존하는 단방향 구조로 간다.
...
- 5번 워크플로우가 4번을 **Tool로 호출**할 경우 의존 단방향·코드 순환 없음(의존 채택 시). 독립으로 가도 두 유닛은 분리 유지.
...
### 4.1 계약 게이트 병렬 실행 (D5) — *5→4 의존 확정 시에만 적용*

(아래는 **의존을 채택할 경우**의 실행 패턴이다. 의존을 두지 않으면 4·5는 일반 순차/독립 트랙으로 간다.)
...
- **4 문서가 5 문서보다 꼭 먼저인가?** 아니다 — 두 문서는 대체로 병행 가능. 단 **5→4 의존을 채택(기본 제안)하면**, 5 설계가 4의 **근거 출력 계약(evidence DTO·Tool 포트)** 에 잠기므로 그 **계약 골격만 4 쪽에서 먼저 합의·동결**돼야 한다(나머지 5 자체 설계는 동시 진행). **의존을 안 두기로 하면 순서 무관·완전 병행.**
...
- **구현 병렬 착수 시점**: 페이즈 1~3 가용 **+** (의존 채택 시) 에이전트 Tool 포트·evidence DTO **동결** + 4의 초기 golden fixture 제공 → 그때부터 4·5 동시 구현, 통합 게이트에서 5를 실제 4로 재검증.
- **질문지·user-stories 작성 절차**: 4·5의 인셉션 질문지·요구사항·user-stories는 **각 유닛 오너(4=화랑님·5=석현님)가 각자 병렬로** 작성한다 — 한 사람이 상대 문서를 먼저 대필할 필요 없음. **공동 선행이 필요한 것은 "공유 계약"뿐**(에이전트 Tool 포트·근거 출력 DTO·세션 모델·근거표·UI 네비 = `shared/`). 의존을 두지 않기로 하면 공유 계약 조율도 최소화되어 처음부터 완전 병렬.
...
1. ~~**D2 최종 승인** — 에이전트 2유닛 분리(§4) 확정 여부.~~ → **2026-06-26 2유닛 분리 승인**. **단 5→4 의존 방향·D5 병렬 적용 여부는 미확정 → requirements 질문지에서 확정**(2026-06-28 완화).
