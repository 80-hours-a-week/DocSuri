...
**10/10 확정** (2026-06-10, 사용자·팀 협의). 기준 리전: **ap-northeast-1 (도쿄)** — D3 연쇄로 확정 ([ADR-D3](#adr-d3)).
...
| [D3](../story-artifacts/handoff.md#d-3) | 임베딩 모델 | ✅ 승인 | **Cohere Embed Multilingual v3** (도쿄) → [ADR-D3](#adr-d3) |
...
| [D9](../story-artifacts/handoff.md#d-9) | 호스팅 환경 | ✅ 승인 | **풀 서버리스 (AWS, 도쿄)** → [ADR-D9](#adr-d9) |
...
- **결정**: **(a) 풀 서버리스** — Amplify Hosting(프론트) + Lambda(백엔드, Web Adapter) + DynamoDB(TTL 캐시·텔레메트리·용어집) + Bedrock(LLM·임베딩). 기준 리전 **ap-northeast-1(도쿄)** — 당초 서울 기준 검토였으나 [ADR-D3](#adr-d3) 연쇄(Cohere KB 임베딩의 리전 지원)로 도쿄 통일 (2026-06-10). 한국 사용자 추가 지연 ~30ms는 LLM 호출(수 초) 대비 무시 가능.
- **근거**:
...
- **결정**: **Amazon Bedrock Knowledge Bases + S3 Vectors** (벡터 스토어). S3 Vectors는 서울·도쿄 모두 GA 확인(2026-06-10, [조사 §15 출처](tech-stack-aws-candidates.md)) — 최종 리전은 도쿄 ([ADR-D3](#adr-d3) 연쇄).
...
<a id="adr-d3"></a>
## 5. ADR-D3 — 임베딩 모델: Cohere Embed Multilingual v3 (리전: 도쿄)

- **상태**: ✅ 승인 · **결정일**: 2026-06-10 · **결정자**: 사용자(팀 협의) · **전제**: [ADR-D2](#adr-d2) 결과 4
- **컨텍스트**: ko↔en 교차언어 검색([US-DISC-04](../story-artifacts/user_stories.md#us-disc-04))이 핵심 품질 변수. KB 임베딩 모델은 리전 제약 — 서울=Titan V2만, Cohere Multilingual=도쿄.
- **결정**: **`cohere.embed-multilingual-v3`** (1024차원) + **전 스택 도쿄(ap-northeast-1) 통일** — ADR-D2 결과 4의 (ii)안.
- **근거**: 교차언어 품질 우선 — `KoEnQueryMapper`([component-model §3.2](component-model.md))의 한→영 매핑이 실패하거나 부정확해도 임베딩 자체가 안전망이 된다. 비용 ~$0.1/1M tok로 무시 가능. 도쿄 이동의 대가는 한국 사용자 +~30ms 지연뿐.
- **기각된 대안**: Titan V2+서울(영어 최적화 — 매퍼 단일 실패점) · S3 Vectors 직접 API(청킹·동기화 자체 구현 부담).
- **결과**: S3 Vectors 인덱스 1024차원 생성. `KoEnQueryMapper`는 DISC-04 AC(매핑 1줄 *표시*) 목적 유지, 검색은 원문 임베딩 병행 가능. [ADR-D9](#adr-d9) 리전 표기 도쿄로 갱신됨.
- **NFR 추적**: LANG-01·02 · PERF-01.
...
- **결정**: **Claude Haiku 4.5** — `global.anthropic.claude-haiku-4-5` CRIS, 도쿄 소스 (도쿄는 global CRIS 소스 리전 — [조사 §15](tech-stack-aws-candidates.md)). CRIS 추가 과금 없음.
...
| 포트/정책 | 확정 구현 |
|---|---|
| `EmbeddingPort.embed` | Bedrock InvokeModel — `cohere.embed-multilingual-v3` (1024d, 도쿄) |
