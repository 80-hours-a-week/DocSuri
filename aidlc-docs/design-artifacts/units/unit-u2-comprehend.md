...
> 🔶 **3/5 통과 (부분, 2026-06-12)** — U2 노출 라운드([`u2_ui_build_plan.md`](../../plans/u2_ui_build_plan.md), PR #30) 검증 범위만 체크 (U0 §6 갱신 위임 전례). 검증: pytest 65(mock) + 데모 URL 실 LLM(Haiku) 확인. 잔여 2건의 사유는 각 항목 주석.

- [ ] [U0](unit-u0-foundation.md) `LlmPort`를 canned 응답으로 mock해도 5개 스토리 AC 통과 — *4/5: COMP-01·02·03·04 통과, COMP-05는 데이터 소스(그림 캡션) 부재로 라운드 제외 — PDF 파싱 도입 시*
- [x] 전문/학부 모드 토글이 동일 입력에서 *다른 톤*을 산출 — *mock·실모델 양쪽 확인 (전문: 어휘 원형+병기 / 학부: 풀어쓰기)*
- [ ] [US-COMP-04](../../story-artifacts/user_stories.md#us-comp-04) 데스크톱 드래그·모바일 롱프레스 양쪽 통과 — *데스크톱 드래그 E2E ✓ · 모바일 롱프레스는 구현+≥500ms 검증식 — 실기기 검증 잔여*
- [x] [NFR-UX-01](../../requirements/nfr.md#nfr-ux-01) 가독성 측정 자동 보고서 (학부 모드 평균 문장 길이 ≤22 어절) — *실모델 7.75어절/문장 통과, 초과 시 1회 재작성 로직 동작*
- [x] [NFR-COST-01](../../requirements/nfr.md#nfr-cost-01) 시뮬레이션 — 모드별 평균 토큰·비용 — *[ADR §13](../architecture_decision_record.md) + 실측(전문 533/753 · 학부 675/897 tok, 7d 캐시·CostGuard 하)*
