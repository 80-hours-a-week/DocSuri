...
- [~] Infrastructure Design — **진행 중·리뷰 게이트 (2026-06-19)**. 계획서 `construction/plans/u7-summarization-infrastructure-design-plan.md`(질문 게이트). 기존 인프라 재사용(신규 관리형 0) — S3 prefix/라이프사이클·Redis 키스페이스/TTL·RDS 용어집 테이블 DDL·Bedrock IAM·CI 자격증명·NFR-C1 비용 라인 확정 예정. 산출물·코드 미생성(답변 대기).
