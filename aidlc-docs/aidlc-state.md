...
- **현재 단계**: CONSTRUCTION - 재인셉션 페이즈 1 / U1 Corpus Infrastructure Design 완료(리뷰 게이트, PR #225 DocModel 계약 보정 반영). 승인 시 다음: U1 Code Generation.
...
- [x] **PR #225 리뷰 보정 — DocModel 완성형 계약 (2026-06-26)**. 사용자 리뷰에 따라 DocModel을 `fullText` 전문 텍스트 투영본 + `sections[].blocks[]` 멀티모달 구조(paragraph/table/formula/figure/list/code)로 보정. 이미지 바이트/base64/서명 URL은 DocModel에 넣지 않고 `figure.assetRef.assetId`로 `assets/` 저장소를 참조한다. `shared/dtos/docmodel.schema.json`, generated Python DTO, parser projection, U7 structured translation/fullText 재투영, frontend curated type/mock, 관련 테스트와 shared/U1 설계 문서를 갱신. **U1 Code Generation 단계는 아직 시작하지 않음.**
